...
st.title("MADS Milestone 1 - NYC Taxis and Precipitation")
...
st.title(" ")
st.markdown(
    """We wanted to get an overall sense of the taxi activity in NYC for pickups and drop-offs, 
...
st.info(
    "Manhattan had by far the most pickups; JFK and LaGuardia airports were comparable to some of the Manhattan zones"
)
...
st.info(
    "Manhattan still had the most activity, but you can see the dropoffs radiated out from Manhattan and are more spread out."
)
...
rides = (
    alt.Chart(df_rides, title="Rides all year")
    .mark_line()
    .encode(
        x=alt.X(
            "pickup_date:T",
        ...
        strokeDash=alt.StrokeDash(
            "post_treatment_time_dummy:O", title="Afternoon Dummy"
        ),
    )
    .properties(width=875, height=475)
    .interactive()
)
...
st.markdown(
    "To investigate this relationship between rain and number of taxi rides, we created a script to identify days with rain events, and more specifically, found when these rain events began. \
...
st.markdown(
    "We picked January 17th because this day had a rainstorm that started around 12:00 pm, \
...
df_single_12hrs["b_ts_dt"] = pd.to_datetime(
    df_single_12hrs["b_ts"], format="%Y-%m-%d %H:%M:%S"
)
df_single_12hrs["pickup_datetime_1min_dt"] = pd.to_datetime(
    df_single_12hrs["pickup_datetime_1min"], format="%Y-%m-%d %H:%M:%S"
)
df_single_12hrs["pickup_datetime_same_time_dt"] = pd.to_datetime(
    df_single_12hrs["pickup_datetime_same_time"], format="%Y-%m-%d %H:%M:%S"
)
...
line = (
    alt.Chart(pd.DataFrame({"x": [str(b)]}))
    .mark_rule()
    .encode(
        x=alt.X(
            "x:T",
...
rides = (
    ...
    .properties(width=875, height=450)
    .interactive()
)
st.altair_chart(rides + line)
st.info(
    "Here you can you see a spike in rides began around 12:30pm that is sustained to around 2:30pm. The previous and following week are overlayed to help illustrate inherent trends"
)
...
st.markdown("\n\n")
st.markdown(
    "On this particular day, you can see a large percentage increase in the number of rides \
...
st.info(
    "Taxi pickups broadly increased across Manhattan after the rain event began on 01/17, especially over mid to lower Manhattan with many increasing over 50%"
)
st.title(" ")
st.markdown(
...
st.info(
    "Press the play button on the bottom right to view the time lapse. This plot was generated using kepler.gl"
)
st.title(" ")
...
st.info(
    "Here is an arc map of the same time lapse on January 17th; for each taxi ride, the pickup location (in blue) is connected to its drop off location (in yellow) via an arc. Newark, LaGuardia and JFK airports can be made out."
)
st.title(" ")
...
df_sing_3w_4hrs = df_single_12hrs[
    (df_single_12hrs["pickup_datetime_same_time_dt"] >= ts_before)
    & (df_single_12hrs["pickup_datetime_same_time_dt"] <= ts_after)
]
# current week
...
lstDictsBID = []
treatment_pre = df_sing_3w_cw[df_sing_3w_cw["pickup_datetime_same_time_dt"] < b][
    "rides_per_minute"
].mean()
...
lstDictsBID.append(
    {
        ...
        "b": b,
        "which": "treatment",
        "timestamp": b + timedelta(hours=-1),
        "rides_per_min": treatment_pre,
    }
)
treatment_post = df_sing_3w_cw[df_sing_3w_cw["pickup_datetime_same_time_dt"] >= b][
    "rides_per_minute"
].mean()
lstDictsBID.append(
    {
...
control_ow_pre = df_sing_3w_ow[df_sing_3w_ow["pickup_datetime_same_time_dt"] < b][
    "rides_per_minute"
].mean()
lstDictsBID.append(
    {
        ...
        "which": "control",
        "timestamp": b + timedelta(hours=-1),
        "rides_per_min": control_ow_pre,
    }
)
control_ow_post = df_sing_3w_ow[df_sing_3w_ow["pickup_datetime_same_time_dt"] >= b][
    "rides_per_minute"
].mean()
lstDictsBID.append(
    {
...
lstDictsBID.append(
    {
        ...
        "which": "counter-factual",
        "timestamp": b + timedelta(hours=1),
        "rides_per_min": treat_counter_fact,
    }
)
...
rides = (
    alt.Chart(
        df_sing_3w_4hrs,
        title="Rides in two-hour period before and after "
        + str(b.strftime("%A"))
        + " - "
        + str(b),
        height=120,
...
did_points = (
    ...
    .encode(
        ...
        color=alt.Color(
            "which:N",
            title="Rides per minute means",
            scale=alt.Scale(
                domain=["control", "treatment", "counter-factual"],
                range=["red", "blue", "blue"],
            ),
            legend=None,
        ),
...
did_lines = (
    ...
    .encode(
        ...
        color=alt.Color(
            "which:N",
            title="Rides per minute means",
            scale=alt.Scale(domain=["control", "treatment"], range=["red", "blue"]),
        ),
    )
)

count_fact_line = (
    alt.Chart(df_did[df_did["which"] == "counter-factual"])
    ...
    .encode(
        ...
        color=alt.Color(
            "which:N",
            scale=alt.Scale(domain=["counter-factual"], range=["blue"]),
            legend=None,
        ),
        strokeDash=alt.StrokeDash(
            "which:N",
            scale=alt.Scale(domain=["counter-factual"], range=[[2, 2]]),
            legend=alt.Legend(title="Diff-in-Diff"),
        ),
    )
)
...
text_res = (
    alt.Chart(dftext_res)
    .mark_text(color="black", align="left", dx=-100, dy=-80, fontSize=15)
    .encode(x="tx:T", y=alt.Y("ty:Q"), text="text")
)

dftext_pre = pd.DataFrame(
    {"text": [t_pre, c_pre], "tx": [x_pre, x_pre], "ty": [280, 315]}
)

text_pre = (
    alt.Chart(dftext_pre)
    .mark_text(color="black", align="left", dx=-45, dy=0, fontSize=15, fontWeight=700)
    .encode(x="tx:T", y=alt.Y("ty:Q"), text="text")
)
...
text_post = (
    alt.Chart(dftext_post)
    .mark_text(color="black", align="left", dx=10, dy=0, fontSize=15, fontWeight=700)
    .encode(x="tx:T", y=alt.Y("ty:Q"), text="text")
)
...
st.info(
    "Here we applied the Differences in Differences casual inference technique to the rain event on January 17th. The fitted lines are for illustrative puprposes only, and all computations were performed seperately."
)
...
df_weather_rides = pd.merge(
    df_weather,
    df_rides,
    how="left",
    left_on=["DATE", "afternoon_dummy"],
    right_on=["pickup_date", "post_treatment_time_dummy"],
)
df_reg = (
    df_weather_rides.groupby(
        [
            "DATE",
            "afternoon_dummy",
            "HourlyPrecipitation",
...
with st.echo():
    reg = smf.ols(
        formula="RidesCount ~ Tuesday + Wednesday + Thursday + Friday + Saturday + Sunday +  HourlyDryBulbTemperature + treatment + afternoon_dummy + treatment:afternoon_dummy",
        data=df_ols,
    ).fit()
...
points = base.mark_point(filled=True).encode(
    ...
    y=alt.Y(
        "Covariate:N", sort=alt.EncodingSortField(field=sortCol, order="descending")
    ),
...
st.markdown("\n\n")
st.info("Visual results of the regression with 95% confidence bars.")
st.markdown(
    """The results of our regression were not statistically significant, as you can see the 95% confidence interval of the
...
st.info(
    "Tips for rides paid via credit card were on average higher outside of Manhattan. Note the two outlier zones in Manhattan for zones Upper East Side North and Union SQ "
)
st.markdown("\n\n")
st.title(" ")
st.markdown(
    "We expected taxi rides paid in cash to under report tips, but to our surprise, the under-reporting was much more egregious than expected."
)
st.markdown(" ")
st.markdown(" ")
...
st.info(
    "Taxi rides paid via cash dramatically underreporrted tips, or didn't report them at all"
)
